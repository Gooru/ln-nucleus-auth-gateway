...
class RouteSchoolConfigurator implements RouteConfigurator {
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.get(RouteConstants.EP_NUCLUES_AUTH_SCHOOL).handler(this::getSchools);
  }

  private void getSchools(RoutingContext routingContext) {
    DeliveryOptions options =
            new DeliveryOptions().setSendTimeout(mbusTimeout).addHeader(MessageConstants.MSG_HEADER_OP, CommandConstants.LIST_SCHOOL);
    eb.send(MessagebusEndpoints.MBEP_SCHOOL, RouteRequestUtility.getBodyForMessage(routingContext), options, reply -> {
