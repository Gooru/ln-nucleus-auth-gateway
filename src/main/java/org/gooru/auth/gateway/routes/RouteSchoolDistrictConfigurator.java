...
class RouteSchoolDistrictConfigurator implements RouteConfigurator {
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.get(RouteConstants.EP_NUCLUES_AUTH_SCHOOL_DISTRICTS).handler(this::getSchoolDistricts);
  }

  private void getSchoolDistricts(RoutingContext routingContext) {
    DeliveryOptions options =
            new DeliveryOptions().setSendTimeout(mbusTimeout).addHeader(MessageConstants.MSG_HEADER_OP, CommandConstants.LIST_SCHOOL_DISTRICT);
    eb.send(MessagebusEndpoints.MBEP_SCHOOL_DISTRICT, RouteRequestUtility.getBodyForMessage(routingContext), options, reply -> {
