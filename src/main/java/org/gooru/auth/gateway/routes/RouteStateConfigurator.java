...
class RouteStateConfigurator implements RouteConfigurator {
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.get(RouteConstants.EP_NUCLUES_AUTH_STATE).handler(this::getStates);
  }

  private void getStates(RoutingContext routingContext) {
    DeliveryOptions options =
            new DeliveryOptions().setSendTimeout(mbusTimeout).addHeader(MessageConstants.MSG_HEADER_OP, CommandConstants.LIST_STATE);
    eb.send(MessagebusEndpoints.MBEP_STATE, RouteRequestUtility.getBodyForMessage(routingContext), options, reply -> {
