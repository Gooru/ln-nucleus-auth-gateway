...
final class AnonymousACLVerifier {
	...
	/**
	 * This mapper constant have route suffix url and it's request methods which are
	 * allowed by gateway using anonymous token.
	 */
	static {
	...
	private static boolean permittedBasedOnRouteAndHttpMethod(HttpServerRequest httpServerRequest) {
		...
		for (Entry<String, List<String>> entry : ROUTES.entrySet()) {
			if (path.endsWith(entry.getKey()) && entry.getValue().contains(httpServerRequest.method().name())) {
				return true;
			}
		}
		return false;
