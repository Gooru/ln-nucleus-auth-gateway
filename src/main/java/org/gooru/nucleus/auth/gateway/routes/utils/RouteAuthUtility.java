...
public final class RouteAuthUtility {
    ...
    public static boolean isAuthCheckRequired(HttpServerRequest request) {
        return !((request.method().name().equalsIgnoreCase(HttpMethod.POST.name())) && (
            request.uri().endsWith(RouteConstants.AUTHORIZE) || request.uri().endsWith(RouteConstants.TOKEN) || request
                .uri().endsWith(RouteConstants.SIGNIN))) && !(
            request.method().name().equalsIgnoreCase(HttpMethod.GET.name()) && request.uri()
                .contains(RouteConstants.EP_NUCLUES_AUTH_GOOGLE_DRIVE_CALLBACK));
    }
}
